import { Component, FC, ReactNode, Suspense, useRef, useLayoutEffect, useEffect, useMemo } from 'react';
import { Canvas, useFrame, useThree, invalidate } from '@react-three/fiber';
import { OrbitControls, useProgress, Html, Environment, ContactShadows, useGLTF } from '@react-three/drei';
...
> {
  ...
  componentDidCatch(error: unknown) {
    console.error('ModelViewer failed to render/load:', error);
  }
        ...
        this.props.fallback ?? (
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {message && <div style={{ opacity: 0.9, fontSize: 12 }}>{message}</div>}
            </div>
          </div>
        )
      );
...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  const content = useMemo<THREE.Object3D | null>(() => {
    if (ext !== 'glb' && ext !== 'gltf') {
      console.error('Unsupported format (ModelViewer supports .glb/.gltf only):', ext);
      return null;
    }
  ...
  useLayoutEffect(() => {
    ...
    outer.current.rotation.set(fixedPitch ?? initPitch, fixedYaw ?? initYaw, 0);

    if (autoFrame && fixedZoom == null && (camera as THREE.PerspectiveCamera).isPerspectiveCamera) {
    ...
    // Store initial base position for fixed zoom/rotation mode
    const ndc = pivotW.current.clone().project(camera);
    ndc.x += xOff;
    ndc.y += yOff;
    basePos.current = ndc.unproject(camera);
  ...
  useFrame((_, dt) => {
    ...
    // Use locked base position when fixedZoom is set, otherwise recalculate
    if (fixedZoom != null && basePos.current) {
      const offsetX = cPar.current.x * (camera as THREE.PerspectiveCamera).aspect * 0.5;
      const offsetY = cPar.current.y * 0.5;
    ...
    if (fixedYaw != null || fixedPitch != null) {
      outer.current.rotation.x = (fixedPitch ?? initPitch) + cHov.current.x;
      outer.current.rotation.y = (fixedYaw ?? initYaw) + cHov.current.y;
      need = true;
    } else {
    ...
    // Lock camera at the end of frame to prevent any zoom drift
    if (fixedZoom != null) {
      camera.position.set(0, 0, fixedZoom);
      need = true;
    }
...
const ModelViewer: FC<ViewerProps> = ({
  ...
  const fixedYaw = fixedRotationX == null ? undefined : deg2rad(fixedRotationX);
  const fixedPitch = fixedRotationY == null ? undefined : deg2rad(fixedRotationY);
  const camZ = fixedZoom != null ? fixedZoom : Math.min(Math.max(defaultZoom, minZoomDistance), maxZoomDistance);
  const minZoom = fixedZoom != null ? fixedZoom : minZoomDistance;
  const maxZoom = fixedZoom != null ? fixedZoom : maxZoomDistance;
  const zoomEnabled = fixedZoom == null && enableManualZoom;
    ...
    >
      ...
      <ModelViewerErrorBoundary>
        <Canvas
          ...
          style={{ touchAction: zoomEnabled ? 'pan-y pinch-zoom' : 'pan-y' }}
        >
          ...
          {!isTouch && fixedZoom == null && (
            <DesktopControls pivot={pivot} min={minZoom} max={maxZoom} zoomEnabled={zoomEnabled} />
          )}
        </Canvas>
      </ModelViewerErrorBoundary>
    </div>
